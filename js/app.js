        ...
        if (nameInput.value && (urlInput.value || (fileInput.files && fileInput.files.length > 0))) {
            ...
            try {
                ...
                if (fileInput.files && fileInput.files.length > 0) {
                    ...
                    try {
                        // Tenta link direto primeiro
                        parsedData = await M3UParser.fetchAndParse(urlInput.value);
                    } catch (e) {
                        console.warn("Direct fetch failed (likely CORS), trying proxies...", e);
                        // Tenta com proxy CORS como fallback
                        const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(urlInput.value)}`;
